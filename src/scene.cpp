...
Scene& Scene::import(string path) {
  ...
  Assimp::Importer imp;
  auto scene = imp.ReadFile(path, aiProcess_Triangulate | aiProcess_FlipUVs |
...
void Scene::render() {
  printf("rendering\n");
  Light light;
  light.create(0.1f, 120.f, 60.f, glm::vec3(-15.f, 50.f, -10.f),
  ...
  while (win.poll()) {
    ...
    light.update_position(
        glm::angleAxis(win.dt * 0.2f, glm::vec3(0.f, 1.f, 0.f)) *
        light.position);
    {
      ...
      for (int i = -5; i < 5; i += 2) {
        for (int j = -5; j < 5; j += 2) {
          for (int k = 10; k < 20; k += 2) {
            ...
            auto body = simulator.add_body(mesh->bbMin, mesh->bbMax,
                                           glm::vec3(i, k, j), glm::vec3());
...
Scene::Scene() : win("OGL") {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    cameras.push_back(new Camera(
        60.f, 16.f / 9.f, glm::vec3(cosf(angle) * r, 20, sinf(angle) * r),
        glm::vec3(0)));
  }
  ...
  shadow_shader.load(base + "/shaders/shadow.vert",
                     base + "/shaders/shadow.frag");
